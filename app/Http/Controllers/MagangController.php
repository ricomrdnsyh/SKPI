...
class MagangController extends Controller
{
    ...
    public function create()
    {
        if ($this->checkPengajuanProcessing()) {
            return redirect()->route('mahasiswa.magang.index')->with('error', 'Data tidak dapat dimodifikasi karena pengajuan SKPI sedang diproses atau sudah disetujui.');
        }
        return view('mahasiswa.magang.create');
    }
    ...
    public function store(StoreMagangRequest $request)
    {
        ...
        if ($request->hasFile('file_bukti')) {
            $data['file_bukti'] = $request->file('file_bukti')->store('bukti_magang', 'public');
        }

        MagangMahasiswa::create($data);
        $this->autoResubmitIfNeeded();

        if (request()->ajax()) { return response()->json(["success" => true, "message" => "Data magang berhasil ditambahkan."]); }
    ...
    public function edit($id)
    {
        ...
        return view('mahasiswa.magang.edit', compact('magang', 'pengajuan', 'itemSteps', 'overallSteps', 'isLocked', 'readonly'));
    }

    public function update(StoreMagangRequest $request, $id)
    {
        $magang = $this->getOwnedItem($id);
        if ($this->checkPengajuanProcessing($magang)) {
            return redirect()->route('mahasiswa.magang.index')->with('error', 'Data tidak dapat dimodifikasi karena pengajuan SKPI sedang diproses atau sudah disetujui.');
        }

        if ($magang->status === 'approved') {
            return redirect()->route('mahasiswa.magang.index')->with('error', 'Data yang telah disetujui tidak dapat diubah.');
        }
        ...
        if ($request->hasFile('file_bukti')) {
            if ($magang->file_bukti) {
                Storage::disk('public')->delete($magang->file_bukti);
            }
            $data['file_bukti'] = $request->file('file_bukti')->store('bukti_magang', 'public');
        } else {
        ...
        if (request()->ajax()) { return response()->json(["success" => true, "message" => "Data magang berhasil diperbarui."]); }
    ...
    public function destroy($id)
    {
        ...
        if ($magang->status === 'approved') {
            return redirect()->route('mahasiswa.magang.index')->with('error', 'Data yang telah disetujui tidak dapat dihapus.');
        }
        ...
        if (request()->ajax()) { return response()->json(["success" => true, "message" => "Data magang berhasil dihapus."]); }
    ...
    public function datatable(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function ($row) use ($user) {
                ...
                if ($user->role !== 'bak_fakultas') {
                    $btn .= ' ' . '<button type="button" onclick="confirmDelete(\'' . $row->id_magang . '\')" class="btn btn-sm btn-light btn-active-light-danger text-center border-0" data-bs-toggle="tooltip" data-bs-title="Hapus"><i class="fas fa-trash-alt"></i></button>';
                }
                $btn .= '</div>';
                return $btn;
            })
    ...
    private function getOwnedItem($id): MagangMahasiswa
    {
        $row = DB::table('magang_mahasiswa')->where('id_magang', $id)->first();
        if (!$row) {
