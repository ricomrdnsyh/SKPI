...
@section('content')
    <div class="space-y-6 animate-fade-in max-w-2xl mx-auto">
        <div>
            ...
            <h2 class="page-title">Tambah Data Fakultas</h2>
        </div>

        <div class="card p-6">
            <form action="{{ route('fakultas.store') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label for="nama_fakultas" class="form-label">Nama Fakultas</label>
                    <input type="text" name="nama_fakultas" id="nama_fakultas" required class="form-input">
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div>
                        <label for="kode_fakultas" class="form-label">Kode Fakultas</label>
                        <input type="text" name="kode_fakultas" id="kode_fakultas" class="form-input">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="dekan" class="form-label">Nama Dekan</label>
                        <input type="text" name="dekan" id="dekan" class="form-input">
                    </div>
                </div>
                <div>
                    <label for="nidn_dekan" class="form-label">nidn Dekan</label>
                    <input type="text" name="nidn_dekan" id="nidn_dekan" class="form-input">
                </div>
